...
Written against 4.3, revised on the move to **4.4**, which closed one of the findings below — (i), the live field set — outright; again on the move to **4.5.0**, which reopened (b) from the other end by merging readable ranges that touch, and every GivEnergy bank touches its neighbour; and again on **4.5.1**, which reverted that and closed (b) instead. Where a finding changed, the note says so rather than being deleted.
...
**4.5.0 briefly made this a real constraint, and 4.5.1 took it back.** For one release `DeviceRanges.merged` coalesced maps that *touch*, so any component whose map passed through a `ComponentGroup` got its banks joined into one run — and GivEnergy's banks all touch, because the pages are contiguous. Measured across the eleven families, that turned seven bank-aligned blocks into blocks spanning two banks (`HR(199,59)` crosses 239|240, `HR(258,60)` crosses 299|300, gateway `IR(1700,59)` crosses 1719|1720, and so on), for a net saving of **one request** — on the gateways, and nowhere else. Nothing in the captures shows this hardware answering a read across a bank boundary; the one refusal recorded at an unaligned base, `IR(236,60)` at `0x32`, is exactly that shape. Neither escape hatch applied: a **deliberate gap** needs an address no field claims between two banks, and `HR(299)` and `HR(300)` are both modelled, while **`max_span`** is a width cap, not an alignment rule. The workaround was to stop pooling at all — `components_for()` returned bare components and each drove its own plan.

**4.5.1 keeps every boundary any map draws through a merge**, which is the fix this note asked for, so the workaround is gone and `components_for()` returns a `ComponentGroup` again. Re-measured with the mock's `read_events` across all eleven families: 4.5.1 pooled is **byte-identical to 4.4 pooled** for every family — 58 requests in total, the same 58 the unpooled plans issue, and the same 58 per bank — and no planned block crosses a boundary in any of them. `test_no_planned_block_crosses_a_bank_boundary` now reads through the group, so it is a regression pin on the merge rather than on our own plumbing.

**Nothing left to fix.** The general rule stands, though: a group must not pool components that can fail independently, because one refused block discards the pass.
...
`register_space` is one of `"holding"` or `"input"` per component, but 3 of 11 families here (both inverters and the EMS) declare registers in both. Each becomes *two* components pooled by a `ComponentGroup` — which works correctly, and since 4.5.1 the range merging keeps the bank boundaries intact (see (b)) — but "an inverter" is now two objects and a group, for no reason the device would recognise. The group never saves a request either, since the two members address different spaces and blocks are planned per space; it is bookkeeping for a split the device does not have.
