...
# are both modelled either side of a bank boundary. Nothing else expresses the
# split — a gap needs an address no field claims, and ``max_span`` caps width
# without constraining alignment — so the boundaries stated here are the whole
# of it, and modbus-connection carries them through a merge.
#
...
def components_for(family: str, unit: Any) -> tuple[list[Component], ComponentGroup]:
    ...
    Pooling is safe for the bank boundaries: merging keeps every boundary a
    member's map draws, so no pooled block spans two banks
    (``test_no_planned_block_crosses_a_bank_boundary``).
    """
